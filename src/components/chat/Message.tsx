    ...
    return (
      ...
      >
        <div className={cn(
          ...
          {/* Avatar */}
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-slate-100 to-slate-200 shadow-sm">
            {isUser ? (
              <Icons.User sx={{ fontSize: 18 }} className="text-blue-600" />
            ) : (
              <Icons.Bot sx={{ fontSize: 18 }} className="text-emerald-600" />
            )}
          </div>
          ...
          >
            ...
            {/* Timestamp */}
            <div className={cn(
              "mt-2 text-xs opacity-70 font-medium",
            ...
            {!isTyping && !isUser && (
              <div className="mt-2">
                ...
                >
                  {copied ? (
                    <Icons.Check sx={{ fontSize: 12 }} />
                  ) : (
                    <Icons.Copy sx={{ fontSize: 12 }} />
                  )}
                </Button>
              </div>
            )}
    ...
    <div className="relative my-2">
      ...
      <div className="flex items-center justify-between rounded-t-lg bg-gray-800 px-4 py-2.5">
        ...
        <Button
          ...
          size="sm"
          onClick={handleCopy}
          className="h-7 w-7 p-0 text-gray-400 hover:bg-gray-700 hover:text-white transition-colors"
          aria-label="نسخ الكود"
        >
          {copied ? (
            <Icons.Check sx={{ fontSize: 14 }} />
          ) : (
            <Icons.Copy sx={{ fontSize: 14 }} />
          )}
        </Button>
      </div>

      {/* Code content */}
      <pre className="overflow-x-auto rounded-b-lg bg-gray-900 p-4 text-sm leading-relaxed">
